...
function TableOrderHistory({page, rowsPerPage, onSetPage, orderHistory, handleSortOrder, totalResults}) {
                ...
                <TableHead sx={{backgroundColor: 'rgb(31 41 55 / 1)'}}>
                    <TableRow>
                        ...
                        <TableCell sx={{color: 'white'}}>
                            <TableSortLabelCustom color="#FFF" name="updatedAt" onClick={handleSortOrder}>
                                Updated Date
                            </TableSortLabelCustom></TableCell>
                ...
                <TableBody>
                    {
                        orderHistory.map(order => (
                            <TableRow key={order.id}>
                                ...
                                <TableCell>{moment(order.ordered_at).format('DD/MM/YYYY HH:mm')}</TableCell>
                                <TableCell>{moment(order.updatedAt).format('DD/MM/YYYY HH:mm')}</TableCell>
                                <TableCell><StatusChip status={order.status}/></TableCell>
                                <TableCell>{order.total + order.delivery_fee} VND</TableCell>
                                <TableCell>
                                    <Link style={{ color: "#000000", textDecoration: "underline" }}>View Details</Link>
                                </TableCell>
                            </TableRow>
                        ))
                    }
